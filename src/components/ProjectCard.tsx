...
import { ExternalLink, Code2, Star, Eye, Terminal, ArrowUpRight, Sparkles, Maximize2 } from "lucide-react";
      ...
      >
        ...
        <div className="flex items-center justify-between px-3.5 py-2 border-b border-zinc-800 bg-zinc-950/60 text-xs font-mono text-zinc-400">
          ...
          <div className="flex items-center space-x-2">
            {project.isFeatured && (
              <span className="flex items-center gap-1 text-[11px] font-semibold text-emerald-300 bg-emerald-950/90 border border-emerald-600/60 px-2 py-0.5 rounded-full shadow-sm shadow-emerald-500/20">
        ...
        {/* Thumbnail with Hover Overlay & Interactive Quick Preview */}
        <div className="relative aspect-video w-full overflow-hidden bg-zinc-950">
          ...
          {/* Center Quick Preview Button on Hover */}
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none group-hover:pointer-events-auto">
            <button
              onClick={() => setIsPreviewOpen(true)}
              aria-label={`Pratinjau cepat ${project.title}`}
              className="opacity-0 group-hover:opacity-100 transform scale-90 group-hover:scale-100 transition-all duration-300 inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-zinc-950/90 hover:bg-emerald-950 border border-emerald-500/50 text-emerald-400 hover:text-emerald-300 font-mono text-xs font-semibold shadow-2xl backdrop-blur-md"
            >
